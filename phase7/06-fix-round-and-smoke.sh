...
echo
echo "=== 1) Patch run_fl_round.sh on all edges (fail-fast if Fabric fails) ==="
...
echo
echo "=== 2) Smoke test with a round > last_round (use 4000) ==="
ROUND=1
FABRIC_ROUND=4000

set +e
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no ubuntu@"$VM2_IP" \
  "bash /opt/fl-client/run_fl_round.sh edge-client-1 \$ROUND Org1MSP $PEER1_HOST \$FABRIC_ROUND | tail -n 80"
RC=$?
set -e
echo "EDGE_RC=$RC"

echo
echo "=== 3) peer0.org1 logs (last 3 minutes) ==="
