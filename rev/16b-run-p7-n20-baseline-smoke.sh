...
for cid in $clients; do
  rm -f /opt/fl-client/models/\${cid}_model.npz 2>/dev/null || true
  rm -f /opt/fl-client/logs/fl-ids-\${cid}-r${ROUND}.json 2>/dev/null || true
  rm -f /opt/fl-client/logs/fl_client_\${cid}_r${ROUND}.out 2>/dev/null || true
  rm -f /opt/fl-client/logs/fl_fabric_\${cid}_r${ROUND}.out 2>/dev/null || true
done

for cid in $clients; do
  n=\${cid##*-}
  fr=$((START_FABRIC + n - 1))
  bash /opt/fl-client/run_fl_round.sh \$cid ${ROUND} ${MSP} ${PEER_HOST} \$fr
done
"
...
items=[json.load(open(p)) for p in paths]
