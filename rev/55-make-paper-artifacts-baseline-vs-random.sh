...
IN_RANDOM="${IN_RANDOM:-$(ls -t "$SUM_DIR"/p14_random_5rounds_clean_*.json 2>/dev/null | head -n 1 || true)}"

[ -f "$IN_BASELINE" ] || { echo "ERROR: baseline json introuvable: $IN_BASELINE"; exit 1; }
[ -f "$IN_RANDOM" ] || { echo "ERROR: random json introuvable: $IN_RANDOM"; exit 1; }

TS=$(date -u +%Y%m%d_%H%M%S)
OUT_TABLE="$SUM_DIR/p13_paper_table_baseline_vs_random_${TS}.csv"
OUT_JSON="$SUM_DIR/p13_paper_table_baseline_vs_random_${TS}.json"
OUT_TEX="$SUM_DIR/p13_paper_table_baseline_vs_random_${TS}.tex"
OUT_F1_PNG="$SUM_DIR/p13_fig_f1_baseline_vs_random_${TS}.png"
OUT_FPR_PNG="$SUM_DIR/p13_fig_fpr_baseline_vs_random_${TS}.png"
OUT_ROUNDS_CSV="$SUM_DIR/p13_rounds_baseline_vs_random_${TS}.csv"

python3 - "$IN_BASELINE" "$IN_RANDOM" "$OUT_TABLE" "$OUT_JSON" "$OUT_TEX" "$OUT_F1_PNG" "$OUT_FPR_PNG" "$OUT_ROUNDS_CSV" <<'PY'
...
rows = [
  ...
  ["random(honest)", pm(hon_f1, hon_f1s), pm(hon_fpr, hon_fprs), fmt(g_ipfs_p95,2), fmt(g_tx_p95,2), fmt(g_tot_p95,2)],
  ["random(byz)", pm(byz_f1, byz_f1s), pm(byz_fpr, byz_fprs), fmt(g_ipfs_p95,2), fmt(g_tx_p95,2), fmt(g_tot_p95,2)],
]
...
with open(out_rounds,"w",newline="") as f:
    wr=csv.writer(f)
    wr.writerow(["round","baseline_f1","baseline_fpr","random_honest_f1","random_honest_fpr","random_byz_f1","random_byz_fpr"])
    for r in range(1,6):
        ...
        if not gcsv:
            patt = list(Path(Path(p_g).parent).glob(f"p14_random_round{r:02d}_clients_*.csv"))
...
res = {
  "inputs": {"baseline": p_base, "random": p_g},
  ...
  "random_honest": {"f1_avg": hon_f1, "fpr_avg": hon_fpr},
  "random_byz": {"f1_avg": byz_f1, "fpr_avg": byz_fpr},
}
