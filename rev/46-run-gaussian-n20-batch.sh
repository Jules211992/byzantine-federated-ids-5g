...
for r in $(seq "$START_ROUND" "$END_ROUND"); do
  ...
  echo
  echo "=============================="
  echo "RUN gaussian N=20 round=$r START_FABRIC=$fab_start"
  echo "=============================="
  ...
  out_csv="$RUN_DIR/summary/p13_gaussian_round$(printf '%02d' "$r")_clients_${TS}.csv"
  out_json="$RUN_DIR/summary/p13_gaussian_round$(printf '%02d' "$r")_summary_${TS}.json"
