...
ATTACK_MODE="${ATTACK_MODE:-backdoor}"
ATTACK_DELTA="${ATTACK_DELTA:-10}"

for ip in "${EDGE_IPS[@]}"; do
  echo
  echo "===== SETUP BACKDOOR on $ip ====="
...
out=[]
drop=("BYZ_CLIENTS=","ATTACK_MODE=","ATTACK_SCALE=","ATTACK_SIGMA=","ATTACK_SEED=","ATTACK_DELTA=")
for line in lines:
...
out.append('ATTACK_DELTA='+"${ATTACK_DELTA}")
p.write_text("\n".join(out)+"\n")
print("OK_SET")
PY

echo HOST:\$(hostname) IP:\$(hostname -I | awk '{print \$1}')
grep -E '^BYZ_CLIENTS=|^ATTACK_MODE=|^ATTACK_DELTA=' /opt/fl-client/config.env || true
REMOTE
done
