...
OUT_CSV="$SUM_DIR/p13_compare_baseline_vs_random_5rounds_${TS}.csv"
OUT_JSON="$SUM_DIR/p13_compare_baseline_vs_random_5rounds_${TS}.json"

python3 - "$IN_BASELINE" "$IN_RANDOM" "$OUT_CSV" "$OUT_JSON" <<'PY'
import sys, json, csv
...
res = {
  ...
  "random_honest": {"f1_avg": hon_f1, "fpr_avg": hon_fpr, "delta_f1": hon_f1-base_f1, "delta_fpr": hon_fpr-base_fpr},
  "random_byz": {"f1_avg": byz_f1, "fpr_avg": byz_fpr, "delta_f1": byz_f1-base_f1, "delta_fpr": byz_fpr-base_fpr},
}

with open(out_csv,"w",newline="") as f:
    wr=csv.writer(f)
    wr.writerow(["metric","baseline","random_honest","delta_honest","random_byz","delta_byz"])
